window.BanHangController = function (
        ...
        .then(function (resp) {
          ...
          $http({
            ...
            if (document.getElementById("hinhThuc1").checked === true) {
              ...
              $scope.tienThanhToan =
                $scope.tongTien + resp.data.data.total - $scope.giamGia;
              $scope.phiShip = resp.data.data.total;
            }
            // $scope.phiShip = resp.data.data.total;
  ...
  $scope.apCoupon = function () {
    ...
    $http
      .get(
        "http://localhost:8080/api/getcoupon/" +
          document.getElementById("khachhang").value
      )
      .then(function (resp) {
        ...
        for (let i = 0; i < $scope.listCoupon.length; i++) {
          if (code === $scope.listCoupon[i].code) {
            ...
            $scope.cashCoupon = $scope.listCoupon[i].cash.toLocaleString(
              "vi-VN",
            ...
            if ($scope.listCoupon[i].isType === 0) {
              ...
              $scope.giamGia +=
                $scope.tongTien * ($scope.listCoupon[i].discount * 0.01);
              $scope.couponGiamGia =
                $scope.tongTien * ($scope.listCoupon[i].discount * 0.01);
            }
            ...
            $scope.tienThanhToan =
              $scope.tongTien +
              $scope.phiShip -
              ($scope.couponGiamGia + $scope.voucherGiamGia);
          }
        }
        ...
        if (code === $scope.listVoucher[i].code) {
          ...
          if ($scope.listVoucher[i].isVoucher === false) {
            if ($scope.listVoucher[i].typeVoucher === false) {
              ...
              $scope.giamGia +=
                $scope.tongTien * ($scope.listVoucher[i].discount * 0.01);
              $scope.voucherGiamGia +=
                $scope.tongTien * ($scope.listVoucher[i].discount * 0.01);
            }
            $scope.tienThanhToan =
              $scope.tongTien +
            ...
            $http
              ...
              .then(function (cart) {
                for (let j = 0; j < cart.data.length; j++) {
                  $http
                    .get(
                      "http://localhost:8080/api/productvoucher/getbyproduct/" +
                        cart.data[j].productDetail.product.id
                    )
                    .then(function (resp) {
                      if (resp.data.length > 0) {
                        let Price =
                          cart.data[j].quantity * cart.data[j].unitPrice;
                          ...
                          if (resp.data[i].voucher.code == code) {
                            ...
                            if (resp.data[i].voucher.typeVoucher === false) {
                              if (resp.data[i].voucher.cash > Price) {
                                ...
                                $scope.voucherGiamGia +=
                                  resp.data[i].voucher.cash;
                              }
                            } else {
                              $scope.giamGia +=
                                Price * (resp.data[i].voucher.discount * 0.01);
                              $scope.voucherGiamGia +=
                                Price * (resp.data[i].voucher.discount * 0.01);
                            }
                          }
                      ...
                      if ($scope.listSPVoucher.length === 0) {
                        ...
                        Swal.fire(
                          "Rất tiếc voucher này không áp dụng cho sản phẩm nào trong giỏ hàng của bạn !",
                          "",
                          "error"
                        );
                      } else {
  ...
  if ($location.search().vnp_TransactionStatus === "00") {
    $http
      .put(
        "http://localhost:8080/api/bill/updateStatus1/" +
      ...
      .then(function (response) {
        ...
        setTimeout(() => {
          Swal.fire({
            title:
              "Bạn có muốn in hóa đơn cho đơn hàng " +
              $location.search().vnp_OrderInfo +
              " ?",
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
              ...
              //custom file name
              html2pdf()
                .set({ filename: $location.search().vnp_OrderInfo + ".pdf" })
                .from(element)
                .save();
  ...
  if ($location.search().vnp_TransactionStatus === "02") {
    $http.delete(
      "http://localhost:8080/api/billhistory/deletebillhistory/" +
        $location.search().vnp_OrderInfo
    );
    $http.put(
      "http://localhost:8080/api/bill/updateStatus/" +
        $location.search().vnp_OrderInfo,
      {
        payStatus: 0,
        paymentDate: null,
  ...
  $scope.exportBill = function () {
    ...
    $scope.listItemExport = [];
    $http
      .get("http://localhost:8080/api/bill/getbycode/" + codeBill)
      .then(function (billexport) {
        $scope.billexport = billexport.data;
        $scope.cus = null;
        $scope.thoigian = new Date();
  ...
  $scope.buy = function (code) {
    ...
    if (
      document.getElementById("diachicuthe").style.display == "block" &&
      document.getElementById("diachicuthe").value.trim() === "" &&
      document.getElementById("hinhThuc2").checked === true
    ) {
    ...
    if (
      document.getElementById("nguoimua").style.display === "block" &&
      document.getElementById("tennguoimua").value.trim() === "" &&
      document.getElementById("hinhThuc2").checked === true
    ) {
    ...
    if (
      document.getElementById("nguoimua").style.display === "block" &&
      !regex.test(document.getElementById("sodienthoai").value.trim()) &&
      document.getElementById("hinhThuc2").checked === true
    ) {
    ...
    if (
      ...
      Swal.fire(
        "Tiền thanh toán 0đ nên bạn không được phép thanh toán online !",
        "",
        "error"
      );
      return;
    ...
    let idCustomer =
      document.getElementById("khachhang").value == "0"
        ? 0
        : document.getElementById("khachhang").value;
        ...
        if (document.getElementById("pay1").checked === true) {
          ...
          if (document.getElementById("hinhThuc1").checked === true) {
            ...
            if (document.getElementById("khachhang").value === "0") {
              $http
                .put(
                  "http://localhost:8080/api/bill/updateBillTaiQuay/" +
                    codeBill,
                  {
          ...
          //mua online thanh toán tiền mặt
            ...
            //khách hàng đã có
              ...
              // địa chỉ có sẵn
                ...
                let idAddressCoSan =
                  document.getElementById("diachiCustomer").value;
  ...
  $scope.$watch("tongTien", function () {
    ...
    $http
      .get(
        "http://localhost:8080/api/product/getAllVoucherByMinimun/" +
          $scope.tongTien
      )
      .then(function (resp) {
